	...
	<?php if ( resume_manager_user_can_post_resume() ) : ?>
		...
		<?php foreach ( $resume_fields as $key => $field ) : ?>
			...
			<fieldset class="form <?php echo esc_attr($add_class); ?> fieldset-<?php echo esc_attr( $key ); ?>">
				<label for="<?php echo esc_attr( $key ); ?>"><?php echo $field['label'] . apply_filters( 'submit_resume_form_required_label', $field['required'] ? ' <small>' . esc_html__('(required)', 'workscout' ) . '</small>' : ' <small>' . esc_html__('(optional)', 'workscout' ) . '</small>', $field ); ?></label>
				<div class="field">
