	...
	<?php if ( job_manager_user_can_post_job() ) : ?>
		...
		<?php foreach ( $job_fields as $key => $field ) : ?>
			<fieldset class="form fieldset-<?php echo esc_attr( $key ); ?>">
				<label for="<?php echo esc_attr( $key ); ?>"><?php echo $field['label'] . apply_filters( 'submit_job_form_required_label', $field['required'] ? '<small>' . esc_html__( ' (required)', 'workscout' ) . '</small>' : ' <small>' . esc_html__( '(optional)', 'workscout' ) . '</small>', $field ); ?></label>
